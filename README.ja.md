<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 005 プロジェクトバナー" width="1200">
</p>
...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#4つの出力を支えるひとつの版画ロジック)
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 005 作例 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 005 作例 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 005 作例 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 005 作例 8"></td>
  </tr>
</table>

## 追加 3:4 上下構成作例

上の 16:9 作例と同じ4点の独立素材を、完全な 3:4 上下構成として再生成しました。上部は現実の写真を保ち、下部は Panel 005 の原文プロンプトに従って再構成しています。英語コピーは各写真の内容・空気・含意だけから生成し、全点を個別に確認しました。

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 005 追加上下作例 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 005 追加上下作例 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 005 追加上下作例 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 005 追加上下作例 4"></td>
  </tr>
</table>
...
> 鈍い大形 · 暗い構造場 · 部分的な露出 · 三層の色秩序 · シルクスクリーン × パステル

XXD Panel 005 は、Codex と互換 Agent のための画像生成 Skill です。写真にある本当の身元、動き、機能、感情を保ち、主体を一つか二つの鈍い大形へ圧縮します。さらに元写真の空間と意味から暗い構造場を育て、隠す、切る、包み込む、部分的に見せるという操作で童話版画の物語を組み立てます。
...
**Panel 005** は、原写真の核を保ちながら、次の視覚方向を目指す制作に向いています：**写真を、暗い構造と小さな鮮色が物語る童話版画へ**。まず用途と期待する結果を確認し、そこから構図、サイズ、文字、納品形式を決めます。
...
> XXD Panel 005 でこの画像を処理して、まず最適な構図とサイズを提案して。
...
## すぐに判断：XXD Panel 005 はあなたに合う？

| 知りたいこと | このスタイルの答え |
|---|---|
| **得られる完成作品** | 写真を、暗い構造と小さな鮮色が物語る童話版画へ |
| **ひと目で分かる特徴** | 鈍い大形 · 暗い構造場 · 部分的な露出 · 三層の色秩序 · シルクスクリーン × パステル |
...
```text
元写真の事実を固定 → 一つか二つの鈍い大形へ圧縮 → 元写真から暗い構造場を育てる → 遮蔽と部分露出で語る → 暗／淡／鮮の三層を固定 → シルクスクリーンと粉状パステルで仕上げる
```

無関係な写真に替えても大形、暗場の役割、遮蔽、鮮色の合図、文字がほぼ成立するなら、それは 005 ではありません。

## 完成作品で見分けやすいスタイル特性

- **一つか二つの鈍い大形：** 少なくとも三つの写真固有の手掛かりで身元、動き、機能、感情を保ちます。
- **写真から育つ暗場：** 大きな低明度の場が、容器、道、枠、または密度の殻として明確に働きます。
- **構造による物語：** 遮蔽、切り取り、包み込み、部分露出が情報の順序を作ります。
- **平面的な絵本視点：** 俯瞰圧縮、横移動、正面枠、道の誘導、尺度跳躍、端の切り取りから一つを選びます。
- **三層の色秩序：** 暗い構造が重さ、淡い場が呼吸、小さな高彩度色が読解の合図を担います。
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 005 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-005 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-005 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-005 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-005 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 005 を適用し、共通設定は一度だけ解決
/xxd-panel-005 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを `~/.claude/skills/xxd-panel-005` にリンクできます。インストール後に Agent セッションを再起動してください。
...
- [中国語ランタイムアダプター](references/xxd-panel-005-prompt.zh-CN.md)
- [英語ランタイムアダプター](references/xxd-panel-005-prompt.en.md)
