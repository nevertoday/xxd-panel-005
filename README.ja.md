...
| モード | サイズ方針 | 成果物 |
| --- | --- | --- |
| `top-bottom` | 元画像適応 | 上に完全な元写真、下に 005 の童話版画構造物語。両パネルは元画像サイズを保ち、厳密に 50/50 |
| `left-right` | 元画像適応 | 左に完全な元写真、右に 005 の童話版画構造物語。両パネルは元画像サイズを保ち、厳密に 50/50 |
| `design-only` | 元画像適応 | 元写真を見せず、変化デザインだけを表示。元画像の比率と寸法を維持 |
| `wallpaper-pack` | 端末別4サイズ | スマートフォン、iPad、デスクトップ、子ども用ウォッチの PNG を個別出力 |

優先順位は、正確な指定ピクセル > 指定比率／用途 > 通常モードの元画像適応です。原稿 `005.md` の 3:4 は初期の制作画角であり、現在の暗黙の既定値ではありません。
...
- **独立4作品：** 各端末は元写真だけを参照し、鈍い大形、暗場の役割、遮蔽、鮮色の合図、文字位置をそれぞれ探れます。
...
自動文案は、主体の身元、見える動き、機能、関係、既知の場所、または暗場が担う構造的な物語から短い語句を抽出します。現在の画像と強く結びつけますが、童話を創作したり語呂合わせや反転を強制したりしません。
...
画像モデルが鈍い大形、写真から育つ暗い構造場、遮蔽による物語、三層の色秩序、シルクスクリーンとパステルの素材、構造内の文字を作ります。`scripts/compose_panel.py` は画布計画、厳密な 50/50 ラスター合成、最終寸法、監査だけを担当し、プログラム描画で作品を偽装しません。
...
- [中国語の完全プロンプト](references/xxd-panel-005-prompt.zh-CN.md)
- [英語の完全プロンプト](references/xxd-panel-005-prompt.en.md)
