<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 005 project banner" width="1200">
</p>
...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#four-outputs-one-print-logic)
...
The repository reserves [`assets/examples/`](assets/examples/) for future work. Only finished 005 artwork verified by the project owner will be added; no post or image from another style is used as a placeholder.

Future samples will demonstrate 005's adaptability. Their subjects, metaphors, palette, copy, and canvas ratios will never become generation references or defaults.

## Four outputs, one print logic
...
| --- | --- | --- |
| `top-bottom` | source-adaptive | complete source above, 005 fairy-tale print structural narrative below; both panels retain the source size and split exactly 50/50 |
| `left-right` | source-adaptive | complete source left, 005 fairy-tale print structural narrative right; both panels retain the source size and split exactly 50/50 |
...
Exact user pixels > explicit ratio or destination > source adaptation for ordinary modes. The original `005.md` used a 3:4 creative canvas, but that historical example is not a silent default in the current Skill.
...
- **Independent set:** every device references only the source photograph and may explore different blunt masses, dark-field roles, occlusion structures, signal colours, and type placements.
...
## Copy must belong to the print structure
...
Automatic copy distils one short word or phrase from subject identity, visible action, function, relation, known place, or the structural story performed by the dark field. It should feel inseparable from the current image without inventing a fairy tale or forcing a pun or reversal.
...
The image model creates blunt masses, a source-grown dark structural field, occlusion narrative, three-layer colour, screenprint-and-pastel material, and type embedded in the structure. `scripts/compose_panel.py` only plans canvases, performs exact 50/50 raster composition, finalises dimensions, and audits results. It never fakes artwork with programmatic drawing.
...
- [Chinese full prompt](references/xxd-panel-005-prompt.zh-CN.md)
- [English full prompt](references/xxd-panel-005-prompt.en.md)
- [Original style brief](references/005-source.md)
...
**Let the dark field hide the noise, then let the one true signal light the story.**
